'use strict';

window.testData = {
    full: {
        columns: ['the', 'cake', 'is', 'a', 'lie'],
        rows: [
            {name: 'foo', values: [[0.13, 0.69], [0.84, 0.49], [0.31, 0.97],
                                   [0.75, 0.29], [0.64, 0.9]]},
            {name: 'bar', values: [[0.98, 0.96], [0.13, 0.7], [0.27, 0.64],
                                   [0.17, 0.24], [0.94, 0.3]]},
            {name: 'baz', values: [[0.94, 0.1], [0.39, 0.63], [0.07, 0.27],
                                   [0.98, 0.02], [0.25, 0.94]]},
            {name: 'glo', values: [[0.3, 0.14], [0.39, 0.4], [0.54, 0.23],
                                   [0.35, 0.47], [0.71, 0.71]]}
        ]
    },

    threeRows: {
        columns: ['the', 'cake', 'is', 'a', 'lie'],
        rows: [
            {name: 'foo', values: [[0.13, 0.69], [0.84, 0.49], [0.31, 0.97],
                                   [0.75, 0.29], [0.64, 0.9]]},
            {name: 'bar', values: [[0.98, 0.96], [0.13, 0.7], [0.27, 0.64],
                                   [0.17, 0.24], [0.94, 0.3]]},
            {name: 'glo', values: [[0.3, 0.14], [0.39, 0.4], [0.54, 0.23],
                                   [0.35, 0.47], [0.71, 0.71]]}
        ]
    },

    fourCols: {
        columns: ['the', 'cake', 'a', 'lie'],
        rows: [
            {name: 'foo', values: [[0.13, 0.69], [0.84, 0.49],
                                   [0.75, 0.29], [0.64, 0.9]]},
            {name: 'bar', values: [[0.98, 0.96], [0.13, 0.7],
                                   [0.17, 0.24], [0.94, 0.3]]},
            {name: 'baz', values: [[0.94, 0.1], [0.39, 0.63],
                                   [0.98, 0.02], [0.25, 0.94]]},
            {name: 'glo', values: [[0.3, 0.14], [0.39, 0.4],
                                   [0.35, 0.47], [0.71, 0.71]]}
        ]
    }
}
